...
export default async function handler(req, res) {
  ...
  try {
    ...
    // System instructions to guide Gemini's behavior
    ...
    // Make the backend connection request directly to Google's live AI servers using native fetch
    ...
    // Safely parse the nested structure inside the Gemini response payload
    if (!apiData.candidates || !apiData.candidates[0]?.content?.parts?.[0]?.text) {
      console.error("Unexpected Gemini API response layout:", JSON.stringify(apiData));
    ...
    // Send the structured data cleanly back to extension.js front-end
